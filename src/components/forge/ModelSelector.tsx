...
export default function ModelSelector({ compact = false }: { compact?: boolean } = {}) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute z-50 w-full mt-1 bg-[#0f1420] border border-white/10 rounded-xl shadow-2xl overflow-hidden max-h-80 overflow-y-auto">
          {/* Auto option */}
          <button
            type="button"
            onClick={() => {
              ...
              {providerModels.map((model) => (
                ...
                >
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {!compact && model.tags?.slice(0, 2).map((tag) => (
                      <Badge
                        key={tag}
